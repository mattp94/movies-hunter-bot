...
var movieHandler = function (file) { // Scrap data for movie and return from + movie
    var movie
...
var duplicateHandler = function (result) { // Check if a movie is a duplicate
    var duplicate
...
var insertionHandler = function (result) { // Insert a movie in db (normal or duplicate)
    if (result.duplicate)
...
var posterHandler = function (result) { // Handle poster and thumbnail (download or move it)
    result.poster = false
...
var trailerHandler = function (result) { // Handle trailer (download or move it)
...
var sleepHandler = function (result) { // Do a break if necessary
    if (/code|api|nothing/.test(result.from))
        sleep(config.sleep)
}

var flushHandler = function () {
...
var flush = function (pathname) { // Replace old data with new (tmp)
    var absPath = utility.absolute(pathname)
    var tmpPath = utility.tmp(absPath, true)
