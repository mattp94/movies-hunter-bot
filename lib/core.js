...
const poster = async result => { // Handle poster and thumbnail (download or move it)
    ...
    if (result.movie.data && result.movie.data.poster) {
        ...
        if (/api/.test(result.from)) {
            ...
            await Promise.all([
                fs.move(utility.absolute(result.movie.data.poster.full), utility.absolute(utility.tmp(result.movie.data.poster.full))),
                fs.move(utility.absolute(result.movie.data.poster.thumbnail), utility.absolute(utility.tmp(result.movie.data.poster.thumbnail)))
            ])
        }
    }
}
...
const trailer = async result => { // Handle trailer (download or move it)
    ...
    if (result.movie.data && result.movie.data.trailer) {
        ...
        if (/api/.test(result.from))
            ...
            await fs.move(utility.absolute(result.movie.data.trailer), utility.absolute(utility.tmp(result.movie.data.trailer)))
    }
}
