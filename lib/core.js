...
const movie = async file => { // Get data for movie and return from + movie
    ...
    if (!hasTag.local) {
        ...
        const codes = await extractor.tmdb.codes(file.search)

        if (codes.length > 0) { // At least one code
            const code = codes[0]

            // < Find from CODE >
            if (!hasTag.reset && (movie = database.findOneFromCode('movies', code) || database.findOneFromCode('tmp', code)))
                return { from: 'code', movie: { id: movie.id, files: [file], data: movie.data } }

            const data = await extractor.tmdb.data(codes[0]) // Extract TMDb data from code

            // < Find from API >
            if (data)
                return { from: 'api/tmdb', movie: { id: uuid(), files: [file], data } }
        }
    }
