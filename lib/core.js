...
const poster = async result => { // Handle poster and thumbnail (download or move it)
    ...
    if (result.movie.data && result.movie.data.poster) {
        ...
        if (/api/.test(result.from)) {
            ...
            await extractor.thumbnail(result.movie)
        } else if (!result.duplicate) {
            await Promise.all([
                fs.rename(utility.absolute(result.movie.data.poster.full), utility.absolute(utility.tmp(result.movie.data.poster.full))),
                fs.rename(utility.absolute(result.movie.data.poster.thumbnail), utility.absolute(utility.tmp(result.movie.data.poster.thumbnail)))
            ])
        }
    }
}
...
const trailer = async result => { // Handle trailer (download or move it)
    ...
    if (result.movie.data && result.movie.data.trailer) {
        ...
        if (/api/.test(result.from))
            ...
            await fs.rename(utility.absolute(result.movie.data.trailer), utility.absolute(utility.tmp(result.movie.data.trailer)))
    }
}
...
const init = async () => { // Create tmp directories for thumbnails, posters and trailers if necessary
    await Promise.all([
        fs.mkdir(utility.tmp(config.locations.thumbnails, true)), // Create tmp directory for thumbnails
        fs.mkdir(utility.tmp(config.locations.posters, true)), // Create tmp directory for posters
        fs.mkdir(utility.tmp(config.locations.trailers, true)) // Create tmp directory for trailers
    ])
}
