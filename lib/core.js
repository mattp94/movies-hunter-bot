...
var movieHandler = function (folder) {
    ...
    // Find from INODE >>
    if (movie = database.findOneFromInode('movies', folder.inode) || database.findOneFromInode('tmp', folder.inode))
        return {from: 'inode', movie: _.omitBy({locations: [folder], data: movie.data}, _.isUndefined)} // if movie.data is undefined then we remove it
    // <<

    // Find from SEARCH >>
    if (movie = database.findOneFromSearch('movies', folder.search) || database.findOneFromSearch('tmp', folder.search))
        return {from: 'search', movie: _.omitBy({locations: [folder], data: movie.data}, _.isUndefined)} // if movie.data is undefined then we remove it
    // <<
    ...
    if (codes.length > 0) { // At least one code
        ...
        // Find from CODE >>
        if (movie = database.findOneFromCode('movies', code) || database.findOneFromCode('tmp', code))
...
var duplicateHandler = function (result) {
    ...
    if (duplicate = database.findOneFromSearch('tmp', result.movie.locations[0].search))
        result.duplicate = duplicate
...
var posterHandler = function (result) {
    ...
    if (/api/.test(result.from) && !result.duplicate && 'data' in result.movie && 'poster' in result.movie.data) {
