...
const { chain, isEmpty, orderBy } = require('lodash')
...
const browse = async (directories, filter) => { // Get (filtered) movies & metadata sorted by creation date
        ...
        walker.on('data', file => {
            if (filter(file.path, file.stats)) // Get only interesting file through a filter defined in bot.js
                promises.push(new Promise((resolve, reject) => {
                    utility.ffprobe(file.path).then(streams => { // Get video's metadata with ffprobe
                        resolve({
                            pathname: file.path,
                            stats: file.stats,
                            streams
    ...
    return orderBy(files, 'date', 'desc') // Sort files by creation date
}
...
const metadata = { // Add metadata in a movie
    ...
    quality: (pathname, streams) => { // Add movie's quality (3D / 1080p / 5.1)
        ...
        if (streams) {
            const vStream = chain(streams.filter(stream => stream.codec_type === 'video' && stream.codec_name !== 'mjpeg')).orderBy(['height', 'width']).last().value() // Get the best video stream
            const aStream = chain(streams.filter(stream => stream.codec_type === 'audio' && stream.channels > 5)).orderBy('channels').last().value() // Get the best audio stream
