...
fs.ensureDirSync(utility.abs(path.dirname(config.locations.log)))

const log = lowdb(utility.abs(config.locations.log)) // Load log from json file
...
const success = (result, file, counter, total) => { // Log a successful movie
    const info = {
        date: new Date(),
        from: result.from,
...
const failure = (error, file, counter, total) => { // Log a failed movie
    const info = {
        date: new Date(),
        error,
        file: _.pick(file, ['dir', 'base', 'search'])
