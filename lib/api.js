...
const google = (search, tld, lang) => { // Get Google results from keywords
    ...
    try {
        response = request('GET', `http://www.google.${tld}/search?hl=${lang}&q=${encodeURIComponent(search)}`, {headers: {'user-agent': randomUa.generate()}}) // Send http request with a random user-agent in order to simulate a web browser
        body = response.getBody('utf8')
    } catch (e) {
        throw new error.LoggedError('api/google' + (e.statusCode ? `:${e.statusCode}` : ''))
    }
...
const allocine = resources => code => { // AlloCiné api
    const config = {
        ...
        }, // Sorted by name properties (mandatory)
    ...
    try {
        ...
        throw new error.LoggedError(`api/allocine/${resources}` + (e.statusCode ? `:${e.statusCode}` : ''))
    }
