...
var google = function (search, tld, lang, results) { // Get Google results from keywords
    try {
        var response = request('GET', encodeURI('https://www.google.' + tld + '/search?hl=' + lang + '&q=' + search + '&start=0&sa=N&num=' + results + '&ie=UTF-8&oe=UTF-8&gws_rd=ssl')) // Send http request
    } catch (e) {
    ...
    $('h3.r a').each(function (i, element) { // Get only urls
        var qsObj = querystring.parse($(element).attr('href')) // Extract url
...
var allocine = {
    movie: function (code) { // Get AlloCiné data from movie code
        ...
        try {
            var response = request('GET', url, {headers: {'user-agent': 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0; MSAppHost/1.0)'}}) // Send http request with a special user-agent so as to bypass restriction
        } catch (e) {
    ...
    trailer: function (movieCode, trailerCode) { // Get AlloCiné trailer url from movie code and trailer code
        try {
            var response = request('GET', 'http://www.allocine.fr/video/player_gen_cmedia=' + trailerCode + '&cfilm=' + movieCode + '.html', {headers: {'user-agent': 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0; MSAppHost/1.0)'}}) // Send http request with a special user-agent so as to bypass restriction
        } catch (e) {
