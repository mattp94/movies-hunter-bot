...
const google = (search, tld, lang, total) => { // Get Google results from keywords
    ...
    try {
        response = request('GET', encodeURI(`https://www.google.${tld}/search?hl=${lang}&q=${search}&start=0&sa=N&num=${total}&ie=UTF-8&oe=UTF-8&gws_rd=ssl`, {headers: {'user-agent': randomUa.generate()}})) // Send http request with a random user-agent so as to bypass restriction
    } catch (e) {
...
const duckduckgo = (search, lang) => { // Get DuckDuckGo results from keywords
    let response

    try {
        response = request('GET', encodeURI(`https://duckduckgo.com/html/?q=${search}&kl=${lang}&k1=-1`, {headers: {'user-agent': randomUa.generate()}})) // Send http request with a random user-agent so as to bypass restriction
    } catch (e) {
        throw new error.LoggedError('api/duckduckgo')
    }

    const $ = cheerio.load(response.getBody('utf8')) // Use cheerio for parsing html
    const results = []

    $('.web-result a.result__a').each((i, element) => { // Get only urls
        results.push($(element).attr('href'))
    })

    return results
}

const startpage = (search, lang) => { // Get StartPage results from keywords
    let response

    try {
        response = request('GET', encodeURI(`https://www.startpage.com/do/asearch?query=${search}&language=${lang}`, {headers: {'user-agent': randomUa.generate()}})) // Send http request with a random user-agent so as to bypass restriction
    } catch (e) {
        throw new error.LoggedError('api/duckduckgo')
    }
    ...
    $('.result h3.clk a').each((i, element) => { // Get only urls
        results.push($(element).attr('href'))
    })
...
const allocine = {
    ...
    trailer: (movieCode, trailerCode) => { // Get AlloCiné trailer url from movie code and trailer code
        ...
        try {
            response = request('GET', `http://www.allocine.fr/video/player_gen_cmedia=${trailerCode}&cfilm=${movieCode}.html`, {headers: {'user-agent': randomUa.generate()}}) // Send http request with a random user-agent so as to bypass restriction
        } catch (e) {
