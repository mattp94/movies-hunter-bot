...
const google = async (search, tld, lang) => { // Get Google results from keywords
    ...
    try {
        ...
        throw new LoggedError('api/google', err.statusCode || err.code)
    }
    ...
    $('h3.r a').each((i, element) => { // Get only urls
        ...
        const url = parsed['/url?url'] || parsed[`http://www.google.${tld}/url?url`] || href // Extract url
...
const allocine = resource => async code => { // AlloCiné api
    ...
    try {
        ...
        throw new LoggedError(`api/allocine/${resource}`, err.statusCode || err.code)
    }
