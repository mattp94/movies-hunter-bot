...
var allocine = {
    movie: function (code) { // Get AlloCiné data from movie code
        ...
        try {
            var response = request('GET', url, {headers: {'user-agent': 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0; MSAppHost/1.0)'}})
        } catch (e) {
    ...
    trailer: function (movieCode, trailerCode) {
        try {
            var response = request('GET', 'http://www.allocine.fr/video/player_gen_cmedia=' + trailerCode + '&cfilm=' + movieCode + '.html', {headers: {'user-agent': 'Mozilla/5.0 (compatible; MSIE 10.0; Windows NT 6.2; Trident/6.0; MSAppHost/1.0)'}})
        } catch (e) {
