...
var update = function (original, movie) { // Insert a duplicated movie (folder) without data
    console.log(original)
    // console.log(db.get('tmp').find(original).value())
    db.get('tmp').find(original).get('locations').push(movie.locations[0]).value()
}
