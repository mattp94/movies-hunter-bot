...
const findOneFromInode = (table, inode) =>
    db.get(table).find(movie => db._.findIndex(movie.files, {inode}) !== -1 ? true : false).value() // Find a movie from inode (return undefined if not exists)
