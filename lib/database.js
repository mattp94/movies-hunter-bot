...
var db = lowdb(utility.getAbsolutePath(config.locations.db))

db.defaults({movies: [], buffer: []}).value()
...
var findOneFromId = function (id) { // Find a movie from Id
    return db.get('movies').find({id: id}).value() // return undefined if not exist
}
