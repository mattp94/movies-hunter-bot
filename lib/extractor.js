...
const poster = movie => { // Download poster
    ...
    try {
        ...
        throw new error.LoggedError('extractor/poster' + (e.statusCode ? `:${e.statusCode}` : ''))
    }
...
const thumbnail = movie => { // Generate a thumbnail
    ...
    try {
        ...
        throw new error.LoggedError('extractor/thumbnail')
    }
...
const trailer = movie => { // Download trailer
    ...
    try {
        ...
        throw new error.LoggedError('extractor/trailer' + (e.statusCode ? `:${e.statusCode}` : ''))
    }
