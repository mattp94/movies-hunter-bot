...
const poster = async movie => { // Download poster
    ...
    await fs.outputFile(utility.absolute(utility.tmp(local)), response.body) // Save it in a temporary folder
    movie.data.poster.full = local // Update data with local path of poster
}

const thumbnail = movie => { // Generate a thumbnail
    const src = utility.absolute(utility.tmp(movie.data.poster.full))
