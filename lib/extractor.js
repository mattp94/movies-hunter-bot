...
const allocine = {
    codes: search => { // Extract AlloCiné codes from Google
        const results = api.google(search + ' allocine', 'fr', 'fr') // Google
    ...
    data: code => { // Extract useful data only
...
const poster = movie => { // Download poster
    ...
    try {
        const response = request('GET', url, {headers: {'user-agent': randomUa.generate()}}) // Download with a fake user-agent
        body = response.getBody()
    } catch (e) {
        throw new LoggedError('extractor/poster', e.statusCode || 'ENOTFOUND')
    }
...
const trailer = movie => { // Download trailer
    ...
    try {
        ...
        throw new LoggedError('extractor/trailer', e.statusCode || 'ENOTFOUND')
    }
