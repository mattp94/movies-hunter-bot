...
const allocine = {
    getCodes: search => { // Extract AlloCiné codes from a search engine
        const results = api.google(search + ' allocine', 'fr', 'fr', 5) // Google
        const results = api.duckduckgo(search + ' allocine', 'fr-fr') // DuckDuckGo
        const results = api.startpage(search + ' allocine', 'francais') // StartPage
    ...
    getData: code => { // Extract useful data only
        ...
        if (result) {
            ...
            if (_.has(result, 'synopsis'))
                _.set(data, 'synopsis', striptags(_.get(result, 'synopsis'), ['br'])) // Remove html tags except <br>
            ...
            if (_.has(result, 'poster.href'))
                _.set(data, 'poster.full', _.get(result, 'poster.href').replace('http://images.allocine.fr', 'http://fr.web.img4.acsta.net')) // Use a CDN which is more reliable

            if (_.has(result, 'trailer.code')) {
                const href = api.allocine.trailer(code, _.get(result, 'trailer.code'))

                if (href)
                    _.set(data, 'trailer', href)
            }
