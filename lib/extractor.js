    ...
    getData: function (code) { // Extract only useful data
        ...
        if (result) {
            ...
            if (result.poster && result.poster.href) {
                data.poster = {
                    local: {
                        full: path.format({dir: config.locations.posters, name: code, ext: path.extname(result.poster.href)}),
                        thumbnail: path.format({dir: config.locations.thumbnails, name: code, ext: path.extname(result.poster.href)}),
                    },
            ...
            if (result.trailer && result.trailer.href) {
                ...
                if (href) {
                    data.trailer = {
                        local: path.format({dir: config.locations.trailers, name: code, ext: path.extname(href)}),
                        online: href
                    }
                }
            }
