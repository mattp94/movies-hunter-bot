...
var savePoster = function (movie) { // Download poster
    ...
    fs.writeFileSync(utility.getAbsolutePath(utility.getTmpPath(local)), response.getBody()) // Save it in a temporary folder
...
var saveThumbnail = function (movie) {
    ...
    if (config.download.posters)
        src = utility.getAbsolutePath(utility.getTmpPath(movie.data.poster.full)) // If src is a filename

    try {
        utility.thumbnailer(src, utility.getAbsolutePath(utility.getTmpPath(local)), config.thumbnail.width, config.thumbnail.height) // Save it in a temporary folder
    } catch (e) {
        throw new error.BotError('extractor/saveThumbnail')
    }
...
var saveTrailer = function (movie) { // Download trailer
    ...
    fs.writeFileSync(utility.getAbsolutePath(utility.getTmpPath(local)), response.getBody()) // Save
