...
const poster = async movie => { // Download poster
    ...
    await Promise.all([ // Save it in a temporary folder
        fs.outputFile(utility.abs(utility.tmp(dest.full)), response.body),
        fs.outputFile(utility.abs(utility.tmp(dest.thumbnail)), await thumbnailPromise)
    ])
...
const trailer = async movie => { // Download trailer
    ...
    await fs.outputFile(utility.abs(utility.tmp(dest)), response.body) // Save it in a temporary folder
